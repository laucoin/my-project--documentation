...
| `project_id`         | FK → `projects.id` — the project this activity belongs to                                    |
| `name`               | The name of the activity                                                                     |
| `description`        | An optional description *(nullable)*                                                         |
| `capacity`           | `INT4RANGE` — minimum and maximum number of participants (e.g. `[5,20]`)                     |
| `duration`           | Expected duration of the activity stored as a PostgreSQL `INTERVAL`                          |
| `start_availability` | When the activity becomes available *(nullable)*                                             |
| `end_availability`   | When the activity stops being available *(nullable)*                                         |
