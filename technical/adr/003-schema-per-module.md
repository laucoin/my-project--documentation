...
All three backend modules share a single PostgreSQL instance (one deployment unit). Without a boundary strategy, modules
could freely read each other's tables, creating hidden coupling that would make future extraction into independent
...
- If a module is extracted into an independent service, its schema can be migrated to a dedicated database without
