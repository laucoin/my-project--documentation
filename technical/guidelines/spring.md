...
**Convention: all BFF endpoints are prefixed with `/api/v{X}`. The version is set at the router or controller level,
never duplicated on individual routes.**

```kotlin
// ✅ correct — version declared once at the router level
@Configuration
class ProjectRouter {
	@Bean
...
- Current version is `v1` — increment only on breaking changes to the contract
- Never mix versioned and unversioned routes in the same application
- Internal module-to-module communication goes through inbound ports, not HTTP — versioning applies to BFF routes only
- When introducing a `v2`, keep `v1` routes active until all consumers have migrated
